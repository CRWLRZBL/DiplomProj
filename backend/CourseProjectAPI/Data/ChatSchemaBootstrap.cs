...
/// <summary>
/// Создаёт таблицы чата при первом запуске API, если их ещё нет в БД.
/// </summary>
public static class ChatSchemaBootstrap
{
    public static async Task EnsureTablesAsync(AutoSalonContext context, ILogger logger, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            logger.LogInformation("Chat schema bootstrap completed.");
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Failed to ensure chat tables exist.");
            throw;
        }
    }
}
